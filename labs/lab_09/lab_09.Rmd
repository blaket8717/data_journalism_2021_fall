...
```{r}
wv_ppp_data <- readRDS("data/lab_09.rds")
```

```{r}
april_ppp_data <- wv_ppp_data %>%
...
april_ppp_data %>%
  ...
  labs(
    title = "April 2020 WV PPP Data",
    x = "Date Approved",
    y = "Total Loans",
    caption = "source: SBA PPP Loan Database"
  ) +
  geom_point(aes(x= date_approved, y = total_loans)) +
  geom_text(aes(x= date_approved, y = total_loans, label = total_loans, vjust = 1.3, hjust = .5))
...
```{r}
states_ppp_loan <- read_csv("data/state_ppp_all.csv")
```
...
```{r}
# If you want to see the line graph, you have to run the code on line 81 and 85 before you can run this codeblock 

pop_by_state <- get_acs(geography = "state",
                                  variables = c(total_pop_states = "B01001_001"),
...
states_ppp_loan <- states_ppp_loan %>%
  inner_join(pop_by_state, by=c("state")) %>%
  mutate(per_100 =(total_ppp_loans/estimate)*100000) %>%
  filter(per_100 >= "4444") %>%
  arrange(desc(per_100)) 
  
states_ppp_loan %>%
  ggplot() +
  geom_bar(aes(x=reorder(state ,per_100), weight=per_100), fill = "blue") + 
  coord_flip() + 
  theme_minimal() +
  labs(
    title = "Top 10 PPP Loan States",
    x = "States",
    y = "PPP Loans (per 100,000 people)",
    caption = "source: SBA PPP Loan Database"
  ) 
...
**A3.** I would say the most common trend is that people had their loan forgiven, just slightly under their loan amount. There were only a handful on loans that had a large amount and a low forgiveness amount. What is being shown is the loans that were fully forgiven. Any point under that line is considered to be unforgiving loans. It appears that more loans were forgiven, than not forgiven. 

```{r}
wv_loans <- wv_ppp_data %>%
  filter(amount <= 25000) %>%
