...
A2. 28 different districts in the area.

```{r}
justice_url <- "https://www.justice.gov/criminal-fraud/cares-act-fraud"

justice_districts <- justice_url %>%
  read_html() %>%
  html_elements("p") %>%
  html_text() %>%
  as_tibble() %>%
...
A4. After doing some research, he is a rapper who has been charged for playing a role in a 24 million COVID-Relief fraud scheme. I would send a reporter to cover this story because it is a public figure who is committing something fraud, especially when it involves millions of dollars.
...
A6. These judicial districts cover up a large portion of the country causing the numbers to look smaller as far as quantity throughout all courts, but one court could be flooded with cases. 
...
A7. California
```{r}
index_url <- "https://smussenden.github.io/coursefiles/ppp_scraping_example/index.html"

index_table <- index_url %>%
  read_html() %>%
  html_table()
...
for (x in 1:52) {
  url <- index_table[["url"]][x]
  states_info <- url %>%
  read_html() %>%
  html_table() 
  the_answer[x,] = states_info[[1]]
  print(url)
}
