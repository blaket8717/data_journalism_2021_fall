...
```{r}
# add code here to accomplish the task
maryland_ppp_with_zcta_and_naics <- maryland_ppp_with_naics %>% left_join(maryland_zcta, by=c("zip"="ZCTA5N"))
```

## Answer questions

**Q1** What are the top 5 industries (using the NAICS title) with the most PPP applications in Maryland?
**A1** Taxi Service, Beauty Salons, Full-service Restaurants, Offices of Real Estate Agents and Brokers, and then All Other Personal Services

```{r}
# Write the code to produce your answer here
top_5_industries_maryland_ppp <- maryland_ppp_with_zcta_and_naics %>%
  group_by(title) %>%
  summarise(
    count = n()
  ) %>%
...
**A2** Full-service Restaurants, Offices of Physicians (except Mental Health Specialists), Plumbing, Heating, and Air-Conditioning Contractors, Offices of Dentists, and then Commercial and Institutional Building Construction

```{r}
# Write the code to produce your answer here
top_industries_maryland_ppp <- maryland_ppp_with_zcta_and_naics %>%
...
```{r}
# Write the code to produce your answer here
maryland_ppp_with_zcta_and_naics %>%
  filter(PNHA >= 20) %>%
  group_by(zip) %>%
  summarise(
    count = n()
...
**A5** There total loans are not as high as most people within that zip code. Other applicants are buying larger loans.
...
**A6** The top 2 zip codes 21031 and 20701 are both in popular cities in Maryland (Baltimore and Annapolis). They both account for 101 where there is zero Asian population and that is because they probably don't live in those cities. They could live 20 minutes away from their work. 
...
**A7** Junior Colleges has only one loan. I don't know if that is a good thing or a bad thing, but I would like to research and compare to how many other levels of schools apply for loans. Education is important and we have to see where the finances are going toward. 
