...
international_migration <- read_csv("data/tabula-md-statistical-handbook.csv", skip=2, col_names = c("State/city", "July_1_2011", "July_1_2012", "July_1_2013", "July_1_2014", "July_1_2015", "July_1_2016", "July_1_2017", "July_1_2018", "July_1_2019", "total_amount"))
```
...
international_migration %>%
  mutate(percent_of_total = total_amount/198996) %>%
...
**A2** It was required to divide the total amount of every row by the overall amount of Maryland. From there, we get Suburban Washington Region with a percent 62.83 percent change.
...
**A3** Jurisdiction- Kent, Region- Lower Eastern Shore Region

```{r}
# Write the code to produce your answer here
international_migration %>%
  mutate(percent_change = (July_1_2018-July_1_2017)/July_1_2018) %>%
  arrange(desc(percent_change))
```

**Q4** What's your best guess as to why these declines occurred, and in those area in particular?
**A4** It seems like there was a decline throughout all the data. The meaning 
behind this is because it's near the Eastern shore and most people want to get
away from the water and move back to a more urban area.
...
```{r}
# Write the code to load the CSV file here
domestic_migration <- read_csv("data/tabula-md-statistical-handbook-domestic.csv", skip = 2, col_names = c("State/city", "July_2011", "July_2012", "July_2013", "July_2014", "July_2015", "July_2016", "July_2017", "July_2018", "July_2019", "total"))
```
...
**A7** All over the place. A county like Anne Arundel is on an upward trend with 8,472 total 
whereas, Caroline county is declining with a total of -687. Two completely different trends.
