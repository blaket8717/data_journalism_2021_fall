...
**Answer** Dataframe of tweets posted by politicians. Provides their name, gender, and party affiliation.
...
**Answer** If you repeat the same word, the output only counts it as one, example: dog.
...
**Answer** Breaks apart the tweet word by word and puts them in their respective rows.
...
**Answer** COVID19 and COVID 19 are two different tokens. Surprisingly there are more COVID 19 than COVID19. I wonder if they count the dash in between COVID19.
...
**Answer** Yes! As what I pointed out in number 6, COVID 19 has more than COVID19. A percentage count does more justice than counting it individually.
...
**Answer** The two years a split a part. Interesting enough, COVID19 has decreased in the past year in tweets.
...
**Task** Run the following code and describe the results.
**Answer** COVID 19 takes the top spot. Any tweet made before this year is accounted for in this dataframe. This DF focuses on multiple words, instead of just one.
...
**Task** Run the following code and describe the results.
**Answer** Just the top 10 tweets from this year. Multiple words appear, no solo words. 
...
**Task** Run the following code and describe the results.
**Answer** Now we have both years outputting for two worded phrases that were in tweets. Separated by parties.
...
**Task** Run the following code and describe the results.
**Answer** Now we see two worded phrases from 2020 and 2021 from the Republican House politicans.
...
**Answer** I think effective could be a word that seems like it could be more negative than positive, especially when we are focusing on COVID-19. The word positive as well is positive, that most likely indicates that someone tested positive for COVID-19, so pretty counter-intuitive. 
