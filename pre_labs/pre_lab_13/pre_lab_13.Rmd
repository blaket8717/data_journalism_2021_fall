...
**Answer** States and their data on who they voted for in the 2016 election. Along with that is data centered around small businesses and how many votes were toward Trump. From this data, Clinton voters are getting less amounts of PPP loans.
```{r}
...
**Answer** Dots are trending toward the top right of the chart. More percentage of Trump tend to go up in PPP loans. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** A lot more scattered. This shows percentage of what the average employee got from a PPP loan. The more Trump the state is, the more of a guarantee of more money for an employee in small businesses. 
```{r}
...
**Answer** We are comparing the two graphs and the relationship of the trend in each graph. 
```{r}
reveal_data %>%
...
**Answer** This shows the correlaion test that is done to give a one of the graphs a percentage on their trend.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** Now we see the same thing done in number 6, only this time we are using the data from the amount for each in employee when a loan comes in. 
```{r}
...
**Answer** The mean for each employee and business mean. The mean is based on which state voted for Clinton or Trump, and it shows that Trump states received more. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** A two sample t-test. This is being done for data covering amount for each employee. Same data we saw from number 8.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** Same thing as number 10, only this tie we are looking at the business percentage. Overall, if the state voted for Trump, they are more likely to receive a better PPP loan than a state that voted for Clinton.
```{r}
